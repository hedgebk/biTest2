package bthdg.calc;

// TripleEmaCalculator
public class TEmaCalculator extends EmaCalculator {
    private Double m_lastEmaEmaValue;
    private Double m_lastEmaEmaEmaValue;
    protected Double m_lastTEmaValue;

    public TEmaCalculator(int emaSize, long barSize, long barsMillisOffset) {
        super(emaSize, barSize, barsMillisOffset);
    }

    @Override protected void finishCurrentBar(long time, double price) {
        super.finishCurrentBar(time, price);
        m_lastEmaEmaValue = calcEmaEma();
        m_lastEmaEmaEmaValue = calcEmaEmaEma();

        if ((m_lastEmaValue != null) && (m_lastEmaEmaValue != null) && (m_lastEmaEmaEmaValue != null)) {
            // T-EMA = (3*EMA – 3*EMA(EMA)) + EMA(EMA(EMA))
            double tEma = 3 * m_lastEmaValue - 3 * m_lastEmaEmaValue + m_lastEmaEmaEmaValue;
            m_lastTEmaValue = tEma;
        }
    }

    private Double calcEmaEma() {
        return calcEmaEma(m_lastEmaValue);
    }

    private Double calcEmaEma(Double value) {
        if (value == null) {
            return null;
        }
        double prevValue = (m_lastEmaEmaValue == null) ? value : m_lastEmaEmaValue;
        // EMA: {Close - EMA(previous day)} x multiplier + EMA(previous day).
        double ema = (value - prevValue) * m_multiplier + prevValue;
        return ema;
    }

    private Double calcEmaEmaEma() {
        return calcEmaEmaEma(m_lastEmaEmaValue);
    }

    private Double calcEmaEmaEma(Double value) {
        if (value == null) {
            return null;
        }
        double prevValue = (m_lastEmaEmaEmaValue == null) ? value : m_lastEmaEmaEmaValue;
        // EMA: {Close - EMA(previous day)} x multiplier + EMA(previous day).
        double ema = (value - prevValue) * m_multiplier + prevValue;
        return ema;
    }
}
